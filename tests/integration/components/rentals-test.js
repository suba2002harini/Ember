...
module('Integration | Component | rentals', function (hooks) {
  ...
  test('it renders all given rental properties by default', async function (assert) {
    ...
    assert.dom('.container-fluid input').exists();

    assert.dom('.container-fluid .list-group').exists();
    assert.dom('.container-fluid .list-group li').exists({ count: 3 });

    assert
      .dom('.container-fluid .list-group li:nth-of-type(1)')
      .containsText('Grand Old Mansion');

    assert
      .dom('.container-fluid .list-group li:nth-of-type(2)')
      .containsText('Urban Living');

    assert
      .dom('.container-fluid .list-group li:nth-of-type(3)')
